...
LOG_FILE="$LOGS_DIR/$(get_log_filename "test_basic_orientation")"
...
print_test_line "Arrow right conversion test" | tee -a "$LOG_FILE"
TOTAL_TESTS=$((TOTAL_TESTS+1))
...
# Extract path data from right arrow SVG
right_arrow_path=$(grep -o 'd="[^"]*"' "$OUTPUT_DIR/arrow_right_normal.svg" | head -1 | sed 's/d="\([^"]*\)"/\1/')
echo "Right arrow path: $right_arrow_path" >> "$LOG_FILE"

# Extract tip and shaft points
tip_x=$(echo "$right_arrow_path" | grep -o "L 80[\.0-9]* 50[\.0-9]*" | sed 's/L \([0-9\.]*\) .*/\1/' | awk '{print int($1)}')
shaft_x=$(echo "$right_arrow_path" | grep -o "M 20[\.0-9]* " | sed 's/M \([0-9\.]*\) .*/\1/' | awk '{print int($1)}')
...
up_arrow_path=$(grep -o 'd="[^"]*"' "$OUTPUT_DIR/arrow_up_normal.svg" | head -1 | sed 's/d="\([^"]*\)"/\1/')
echo "Up arrow path: $up_arrow_path" >> "$LOG_FILE"

# Extract tip and shaft points - considering SVG coordinate system
# The tip has the form "L 50[\.0-9]* 80[\.0-9]*"
# The shaft base has the form "M 48[\.0-9]* 20[\.0-9]*" or "L 52[\.0-9]* 20[\.0-9]*"
tip_y=$(echo "$up_arrow_path" | grep -o "L [0-9\.]*,* 80[\.0-9]*" | sed 's/L [0-9\.,]* \([0-9\.]*\)/\1/' | awk '{print int($1)}')
shaft_y=$(echo "$up_arrow_path" | grep -o "[ML] [0-9\.]*,* 20[\.0-9]*" | head -1 | sed 's/[ML] [0-9\.,]* \([0-9\.]*\)/\1/' | awk '{print int($1)}')
